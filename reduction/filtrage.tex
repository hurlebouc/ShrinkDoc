...
\noind Passée cette fonction de filtrage, on se rend compte que l'on peut améliorer la fonction \texttt{s1} en continuant la descente de l'arbre pour trouver les \sts de même type que les \ta trouvé par l'\algo\!\!. On construit alors \texttt{s1}$'$ en itérant $f(\mathtt{s1Depth}(\cdot, d))$ sur chacun des termes de $f(\mathtt{s1Depth}(t, d))$ jusqu'à convergence.
...
\noind En ce qui concerne $f \circ \mathtt{s2Depth}(\cdot, d)$, on a pas besoin de réitérer l'\algo car on travail exclusivement sur les constructeurs du type de la racine : $$ \mathtt{s2}' = f \circ \mathtt{s2Depth}.$$
