...
\noind Pour formaliser la syntaxe, on se muni d'un ensemble infini $\X$ de symboles de \textit{variables} et un ensemble $\sig$ de symboles de signatures. Chaque signature a une \textit{arité}. On peut alors définir la notion de termes de manière inductive :
\begin{itemize}
...
\end{itemize}
L'ensembles de termes est noté $\term$.\\

\noind Enfin, on peut définir, également de manière inductive, l'ensemble $\F$ des formules : étant donné $\predicat$ un ensemble de symboles de prédicats et $\D$ est un ensemble de symboles de domaines, on a que
\begin{itemize}
...
\item les symboles de signature de l'ensemble $\sig$ correspondent à des objet implémentant l'interface \texttt{SignatureInterpretation}. Chacune de ces interprétations doit fournir une méthode \texttt{compute} qui à une liste de termes associe la valeur souhaité de l'interprétation du symbole dans ce cas.
