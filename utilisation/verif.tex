...
\noind On peut interpréter une formule syntaxique issue d'un ensemble de signatures $\S$, de prédicats $\P$ et de domaines $\D$ en lui associant une \textit{interprétation} $I$ qui est la connaissance d'un couple $(\d_I, \ent{\cdot}_I)$ où
\begin{itemize}
...
$$ \forall t \in \T, \; \scal{t \vert \sigma}_I \mbox{est un \texttt{ATerm}}.$$

\item les symboles de prédicats de l'ensemble $\P$ correspondent à des objets implémentant l'interface \texttt{PredicatInterpretation}. Chacune de ces interpretations doit alors fournir une méthode \texttt{isTrue} qui prend en paramètre une liste d'évaluations des termes que le symbole de prédicat $P$ a comme paramètre dans la formule, et qui renvoie \texttt{true} \ssi on décide que $P$ est vrai dans ce cas, et \texttt{false} sinon.

$$ \forall P \in \P, \; \ent{P}_I : \mathtt{ATerm}^n \rightarrow \{0,1\} \mbox{ où $P$ est d'arité $n$}.$$

\item les symboles de signature de l'ensemble $\S$ correspondent à des objet implémentant l'interface \texttt{SignatureInterpretation}. Chacune de ces interprétations doit fournir une méthode \texttt{compute} qui à une liste de termes associe la valeur souhaité de l'interprétation du symbole dans ce cas.

$$ \forall s \in \S, \; \ent{s}_I : \mathtt{ATerm}^n \rightarrow \mathtt{ATerm} \mbox{ où $s$ est d'arité $n$}.$$

\item les symboles de domaines de l'ensemble $\D$ correspondent à des objets implémentant l'interface \texttt{DomainInterpretation}. Chacune de ces interpretations doit fournir quatres méthode :
	\begin{itemize}
	...
	\item \texttt{lighten} qui prend un terme appartenant au domaine et renvoie tous les sous termes que l'on peut construire à partir de celui en paramètre en remplaçant sont constructeur par un constructeur plus \textit{petit}. Cette méthode sera détaillée plus loin dans la partie traitant de la réduction des contre-exemples.
	\item \texttt{chooseElement} qui prend un entier $n$ en paramètre et renvoie un éléments du domaine (toujours sous forme de \texttt{ATerm}) dont la taille este l'ordre de $n$. C'est là que l'on utilise la fonction de génération automatique de termes aléatoires. Cette méthode sera employée lorsque l'on cherchera à démontrer la validité de formule dans l'interprétation donnée. En effet, lorsque le testeur rencontrera une formule de la forme "$\forall x \in D, \; (F)$", il générera des termes $x$ de $D$ qu'il injectera dans la formule $F$ pour vérifier sa validité.
...
\noind On peut même faire plus que cela. Si le processus de validation échoue sur sur une valuation, c'est à dire un contre-exemple, on s'est donné les moyens de \textit{réduire} ce contre-exemple pour éventuellement résoudre le bug plus facilement.
